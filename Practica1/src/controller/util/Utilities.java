...
//        }
    ...
    public static Boolean guardar(SucursalController sc) throws IOException, EmptyException, PositionException {    
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        String json = gson.toJson(sc, SucursalController.class);
        try {
    ...
    public static SucursalController cargarJson() throws FileNotFoundException, IOException, EmptyException, PositionException {
        SucursalController mapeo = new SucursalController();
        Reader lector = Files.newBufferedReader(Paths.get("sucursales.json"));
        Gson gson = new Gson();
        mapeo = (gson.fromJson(lector, SucursalController.class));
        return mapeo;
    }
    public static Boolean guardarTablaAcciones(ColaI cola){
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        String json = gson.toJson(cola, ColaI.class);
        try {
            PrintWriter escritor = new PrintWriter(new File("tablaacciones.json"));
            escritor.write(json);
            escritor.flush();
            escritor.close();
        } catch (Exception e) {
    ...
    public static ColaI cargarJsonActividades() throws IOException {
        ColaI mapeo = new ColaI(20);
        Reader lector = Files.newBufferedReader(Paths.get("tablaacciones.json"));
        Gson gson = new Gson();
        mapeo = (gson.fromJson(lector, ColaI.class));
        return mapeo;
    }
